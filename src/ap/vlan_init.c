...
{
	int ret;
	//  printf("Skon: vlan_if_remove\n");

       
	ret = wpa_auth_release_group(hapd->wpa_auth, vlan->vlan_id);
	if (ret)
		wpa_printf(MSG_ERROR,
			   "WPA deinitialization for VLAN %d failed (%d)",			   vlan->vlan_id, ret);
				...
				struct hostapd_vlan *vlan)
{
	...
	//  printf("Skon: vlan_dynamic_remove\n");
...
int vlan_init(struct hostapd_data *hapd)
{
  //  printf("Skon: vlan_init\n");
	...
	if ((hapd->conf->ssid.dynamic_vlan != DYNAMIC_VLAN_DISABLED ||
	    ...
	    !hapd->conf->vlan) {
	  printf("Skon: dynamic vlans enabled but no (or empty) vlan_file given\n");
	...
	//	printf("Skon vlan_dynamic_add call\n");
				       ...
				       struct vlan_description *vlan_desc)
{
	...
	//	printf("Skon vlan_add_dynamic: %s, %s, %d, %d, %d\n",vlan->ifname,vlan->bridge,vlan_id,vlan->vlan_id,vlan->dynamic_vlan);
